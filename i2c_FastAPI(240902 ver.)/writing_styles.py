STYLE_SPECIFIC_INSTRUCTIONS = {
    '일기': """
    1. 1인칭 시점으로 작성해주세요. ('나는', '내가' 등의 표현 사용)
    2. 시간 순서대로 하루의 경험을 서술해주세요.
    3. 개인적인 감정과 생각을 자세히 표현해주세요.
    4. 일기체에 맞는 구어체를 사용해주세요.
    5. 이미지의 주요 장면이나 객체에 대한 개인적인 느낌을 포함해주세요.
    6. 문장을 작성하면서 앞선 문장과 다른 내용을 다루게 된다면 문단을 나눠주세요.
    7. SEO를 위해 관련 키워드를 자연스럽게 포함해주세요.
    """,
    'SNS 포스팅': """
    1. 친근하고 개인적인 톤으로 작성해주세요.
    2. 각 이미지의 하이라이트나 인상적인 순간을 강조해주세요.
    3. 이모지를 적절히 사용해 글의 분위기를 살려주세요.
    4. 독자의 관심을 끌 수 있는 흥미로운 문구나 질문을 포함해주세요.
    5. 위치 태그나 간단한 해시태그를 글 중간중간에 넣어주세요.
    6. 문장을 작성하면서 앞선 문장과 다른 내용을 다루게 된다면 문단을 나눠주세요.
    """,
    '여행기': """
    1. 생생한 경험담을 전달하는 방식으로 작성해주세요.
    2. 여행지의 분위기, 문화, 음식 등을 상세히 묘사해주세요.
    3. 개인적인 느낌과 경험을 공유하되, 유용한 정보도 함께 제공해주세요.
    4. 시간 순서에 따라 여행 경로를 자연스럽게 서술해주세요.
    5. 독자가 그 장소를 방문하고 싶게 만드는 매력적인 표현을 사용해주세요.
    6. 문장을 작성하면서 앞선 문장과 다른 내용을 다루게 된다면 문단을 나눠주세요.
    7. SEO를 위해 관련 키워드를 자연스럽게 포함해주세요.
    """,
    '중고 판매 설명문': """
    1. 판매자의 입장에서 작성해주세요.
    2. 상품의 특징, 상태, 사용 기간 등을 정확하고 상세하게 설명해주세요.
    3. 상품의 장점을 부각시키되, 과장되지 않도록 주의해주세요.
    4. 구매자가 알고 싶어할 만한 정보(크기, 색상, 기능 등)를 포함해주세요.
    5. 친절하고 신뢰감 있는 톤으로 작성해주세요.
    6. 가격과 거래 방법에 대한 정보도 포함해주세요.
    7. 문장을 작성하면서 앞선 문장과 다른 내용을 다루게 된다면 문단을 나눠주세요.
    """,
    '음식 후기': """
    1. 1인칭 시점으로, 생생한 경험담을 전달하는 방식으로 작성해주세요. 음식을 처음 맛봤을 때의 느낌을 생동감 있게 전달하세요.
    2. 음식의 맛, 질감, 향 등을 상세히 묘사해주세요. 음식의 각 요소를 구체적으로 설명하여 독자가 함께 맛보는 느낌을 주도록 하세요.
    3. 음식의 외관과 제공되는 방식에 대해 설명해주세요. 음식이 어떻게 제공되었고, 어떤 모습이었는지 묘사하세요.
    4. 레스토랑의 분위기와 서비스에 대해 언급해주세요. 음식뿐만 아니라 레스토랑의 분위기와 서비스도 함께 설명하세요.
    5. 개인적인 느낌과 추천 여부를 포함해주세요. 음식에 대한 개인적인 생각과 추천 여부를 명확하게 표현하세요.
    6. 가격과 위치 등 유용한 정보를 포함해주세요. 음식의 가격, 레스토랑 위치 등 독자가 유용하게 생각할 정보를 포함하세요.
    7. SEO를 위해 관련 키워드를 자연스럽게 포함해주세요. 예: "서울 맛집", "베스트 파스타", "합리적인 가격"
    8. 문장을 작성하면서 앞선 문장과 다른 내용을 다루게 된다면 문단을 나눠주세요. 내용이 바뀔 때는 문단을 나눠 가독성을 높이세요.
    """,
    '제품 후기': """
    1. 1인칭 시점으로, 사용자 경험을 생생하게 전달해주세요. 제품을 사용하면서 느낀 점을 생동감 있게 전달하세요.
    2. 제품의 주요 기능과 특징을 상세히 설명해주세요. 제품의 주요 기능과 그 특징을 구체적으로 설명하세요.
    3. 제품의 장점과 단점을 공정하게 평가해주세요. 제품의 장점과 단점을 공정하고 객관적으로 평가하세요.
    4. 제품의 사용 방법과 편의성에 대해 언급해주세요. 제품을 사용하는 방법과 사용하면서 느낀 편의성을 설명하세요.
    5. 제품의 가격 대비 성능을 평가해주세요. 제품의 가격과 성능을 비교하여 평가하세요.
    6. 개인적인 추천 여부와 이유를 포함해주세요. 제품을 추천하는지 여부와 그 이유를 명확하게 설명하세요.
    7. SEO를 위해 관련 키워드를 자연스럽게 포함해주세요. 예: "가성비 좋은 스마트폰", "최고의 무선 청소기", "2024년 최신 모델"
    8. 문장을 작성하면서 앞선 문장과 다른 내용을 다루게 된다면 문단을 나눠주세요. 내용이 바뀔 때는 문단을 나눠 가독성을 높이세요.
    """,
    '장소 방문 후기':"""
    1. 1인칭 시점으로, 방문 경험을 생생하게 전달해주세요. 장소를 방문했을 때의 경험을 생동감 있게 전달하세요.
    2. 장소의 분위기, 디자인, 주요 특징 등을 상세히 묘사해주세요. 장소의 분위기와 주요 특징을 구체적으로 설명하세요.
    3. 장소에서 느낀 개인적인 감정과 경험을 공유해주세요. 장소에서 느꼈던 감정과 경험을 구체적으로 설명하세요.
    4. 장소의 장점과 단점을 공정하게 평가해주세요. 장소의 장점과 단점을 공정하게 평가하세요.
    5. 방문에 필요한 유용한 정보를 포함해주세요. 방문 시간, 위치, 입장료 등 유용한 정보를 포함하세요.
    6. 개인적인 추천 여부와 이유를 포함해주세요. 장소를 추천하는지 여부와 그 이유를 명확하게 설명하세요.
    7. SEO를 위해 관련 키워드를 자연스럽게 포함해주세요. 예: "서울 가볼 만한 곳", "2024년 인기 여행지", "숨겨진 명소"
    8. 문장을 작성하면서 앞선 문장과 다른 내용을 다루게 된다면 문단을 나눠주세요. 내용이 바뀔 때는 문단을 나눠 가독성을 높이세요.
    """,
    '인물 소개': """
    1. 3인칭 시점으로 작성해주세요. ('그는', '그녀는', '○○씨는' 등의 표현 사용)
    2. 인물의 기본 정보(이름, 나이, 직업 등)를 먼저 서술해주세요.
    3. 인물의 주요 업적이나 경력을 구체적으로 설명해주세요.
    4. 인물의 성격이나 가치관에 대한 묘사를 포함해주세요.
    5. 인물의 개인적인 취향이나 흥미를 나타내는 정보를 추가해주세요.
    6. 인물의 주요 활동이나 취미 생활에 대한 구체적인 사례를 들어주세요.
    7. 독자가 인물에 대해 더 잘 이해할 수 있도록 관련된 일화나 에피소드를 서술해주세요.
    8. SEO를 위해 인물과 관련된 키워드를 자연스럽게 포함해주세요.
    """,
    '앨범/음원 소개': """
    당신은 뛰어난 음악 평론가입니다. 주어진 앨범 커버 이미지를 바탕으로 해당 앨범의 아티스트에 대한 소개문을 작성해 주세요. 소개문은 다음 가이드라인을 정확히 따라 작성해 주십시오:
    1. 3인칭 시점으로 작성해주세요. ('그는', '그녀는', '○○씨는' 등의 표현 사용)
    2. 인물의 기본 정보(이름, 나이, 직업 등)를 먼저 서술해주세요.앨범 커버에서 유추할 수 있는 아티스트의 이름, 예상 나이, 음악 장르 등을 포함해 주세요.
    3. 인물의 주요 업적이나 경력을 구체적으로 설명해주세요. 앨범 커버의 디자인, 로고, 제목 등에서 추측할 수 있는 아티스트의 음악적 성과나 경력을 서술해 주세요.
    4. 인물의 성격이나 가치관에 대한 묘사를 포함해주세요. 앨범 커버의 전반적인 분위기나 이미지를 통해 아티스트의 성격이나 음악적 가치관을 추론하여 설명해 주세요.
    5. 인물의 개인적인 취향이나 흥미를 나타내는 정보를 추가해주세요. 앨범 커버의 시각적 요소를 바탕으로 아티스트의 음악 외적인 관심사나 취향을 상상하여 서술해 주세요.
    6. 인물의 주요 활동이나 취미 생활에 대한 구체적인 사례를 들어주세요. 앨범 커버에서 암시되는 아티스트의 음악 스타일이나 활동을 구체적으로 묘사해 주세요.
    7. 독자가 인물에 대해 더 잘 이해할 수 있도록 관련된 일화나 에피소드를 서술해주세요. 앨범 커버의 특징적인 요소를 바탕으로 아티스트와 관련된 가상의 에피소드를 창의적으로 만들어 주세요.
    8. SEO를 위해 인물과 관련된 키워드를 자연스럽게 포함해주세요. 앨범 제목, 예상되는 음악 장르, 시각적 요소에서 연상되는 키워드 등을 소개문에 자연스럽게 녹여주세요.
    """
}
